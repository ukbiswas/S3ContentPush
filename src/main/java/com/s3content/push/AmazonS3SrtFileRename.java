...
public class AmazonS3SrtFileRename {
	...
	private static List<String>				successObjectList;
	
	public static void main(String[] args) throws Exception {		
		Properties properties = new Properties();
		CommonUtility commonUtility = new CommonUtility();
		properties.load(new FileReader(commonUtility.getPropertyFile("s3srt-fileRename.properties")));
		...
		successObjectList = new ArrayList<String>();
		try {
            ...
            do {
                ...
                for (S3ObjectSummary objectSummary : objectListing.getObjectSummaries()) {
                	...
                	if (CommonUtility.isCopyToBeSkipped(sourceObjectKey) && sourceObjectKey.endsWith("/")) {
        				continue;
        			} else if (!sourceObjectKey.contains(".srt?")) {
        				continue;
        			}
                	
                    destinationObjectKey = getNewFileName(sourceObjectKey);
                    ...
                    try {
                    	...
                    	sourceS3client.copyObject(sourceBucket, sourceObjectKey, sourceBucket, destinationObjectKey);
                        //destS3client.putObject(sourceBucket, destinationObjectKey, s3Object.getObjectContent(), objectMetadata);
                        //s3Object.close();
                        successObjectList.add(destinationObjectKey);
                        iSourceObjectLength++;
            ...
            if (!successObjectList.isEmpty()) {
				System.out.println("Objects renamed successfully :");
				for (String successObjectKey :  successObjectList) {
					System.out.println("\n\t : " + successObjectKey);
				}
			}
	...
	public static String getNewFileName(String objectKey) {
		String newFileName = "";
		if (StringUtils.isNotBlank(objectKey) && objectKey.contains(".srt?")) {
			System.out.println("old object key="+objectKey);
			newFileName = objectKey.substring(0, objectKey.indexOf(".srt?")+4);
			System.out.println("new object key="+newFileName);
		}
		return newFileName;
	}
}
